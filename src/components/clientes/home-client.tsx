                        ...
                        <div className="space-y-4">
                          <fieldset className="border rounded-lg p-4 space-y-3 dark:border-slate-700">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              ...
                              <div><label className="text-xs text-slate-500 dark:text-slate-400">WhatsApp</label><Input value={form.whatsapp || ''} onChange={(e) => updateForm('whatsapp', e.target.value)} placeholder="(XX) XXXXX-XXXX" /></div>
                            </div>
                          </fieldset>
                          <fieldset className="border rounded-lg p-4 space-y-3 dark:border-slate-700">
                        ...
                        <div className="space-y-4">
                          <fieldset className="border rounded-lg p-4 space-y-3 dark:border-slate-700">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              ...
                              <div className="flex items-end gap-2">{r.editable.whatsapp && <a href={`https://wa.me/55${r.editable.whatsapp.replace(/\D/g, '').replace(/^0+/, '')}`} target="_blank" rel="noopener noreferrer" className="text-green-600 hover:text-green-700 dark:text-green-400 mb-0.5 shrink-0" title="WhatsApp"><MessageCircle className="size-4" /></a>}<div className="flex-1"><label className="text-xs text-slate-500 dark:text-slate-400">WhatsApp</label><Input value={r.editable.whatsapp || ''} onChange={(e) => handleSave(r.parsed.codigo, 'whatsapp', e.target.value)} className="font-mono" placeholder="(XX) XXXXX-XXXX" /></div></div>
                            </div>
                          </fieldset>
                          <fieldset className="border rounded-lg p-4 space-y-3 dark:border-slate-700">
                            <legend className="text-sm font-semibold text-slate-600 dark:text-slate-400 px-2 flex items-center gap-1.5"><Mail className="size-3.5" />Emails</legend>
